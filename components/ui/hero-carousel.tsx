...
export function HeroCarousel({ config, storeSlug }: { config?: any; storeSlug?: string }) {
    ...
    return (
        <div className="relative w-full overflow-hidden bg-[#05050A] group xl:rounded-[2.5rem] xl:mx-6 2xl:mx-auto max-w-[1400px] xl:mb-10 xl:mt-4 shadow-2xl min-h-[480px] md:min-h-[600px] flex items-center">
            {/* Background Glow */}
            ...
            {displaySlides.map((slide, index) => (
                ...
                >
                    <div className="relative z-20 h-full w-full max-w-[1200px] mx-auto px-5 md:px-12 flex flex-col md:flex-row items-center py-10 md:py-0">
                        {/* Left Content */}
                        <div className="w-full md:w-[55%] text-white pr-0 md:pr-8 mb-6 md:mb-0">
                            <h2 className="text-3xl sm:text-5xl md:text-7xl font-black leading-[1.1] tracking-[-0.04em] text-white">
                            ...
                            <p className="text-sm sm:text-base md:text-xl text-gray-300 font-medium leading-relaxed max-w-lg mt-3 md:mt-4">
                                {slide.description}
                            </p>
                            <div className="pt-5 md:pt-6 flex flex-wrap gap-3">
                                <Link
                                    href={slide.ctaDetails.href}
                                    className="inline-flex items-center justify-center bg-blue-600 hover:bg-blue-500 text-white px-6 md:px-8 py-2.5 md:py-3 rounded-full font-bold text-[14px] md:text-[15px] transition-colors"
                                >
                                    {slide.ctaDetails.text}
                                </Link>
                                <Link
                                    href={slide.ctaDetails.href}
                                    className="inline-flex items-center justify-center bg-transparent border border-gray-600 text-white hover:bg-white/10 px-6 md:px-8 py-2.5 md:py-3 rounded-full font-bold text-[14px] md:text-[15px] transition-colors"
                                >
                                    Más información
                                </Link>
                            </div>
                        ...
                        {/* Right Image — hidden on very small screens, visible from sm up */}
                        <div className="w-full md:w-[45%] h-full flex justify-center items-center relative">
                            <div className="relative w-full max-w-[280px] sm:max-w-[380px] md:max-w-[500px] aspect-[4/3] md:aspect-[3/4] rounded-2xl overflow-hidden shadow-2xl border border-white/5">
                                <img
                                    src={slide.image}
                                    alt={slide.title}
